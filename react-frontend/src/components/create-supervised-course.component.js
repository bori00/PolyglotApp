...
export default class CreateSupervisedCourse extends Component {
    ...
    handleSaveCourse(e) {
        ...
        if (this.checkBtn.context._errors.length === 0) {
            CourseManagementService.saveNewSupervisedCourse(this.state.title, this.state.language, this.state.wordTarget)
                .then(response => {
                        if (response.ok) {
                            response.json().then(response => {
                                this.setState({
                                    successful: true,
                                    joiningCode: response.joiningCode
                                })
                            })
                        } else {
        ...
        return (
            <div className="col-md-12">
                <div className="card card-container">
                    ...
                    {!this.state.loading && !this.state.successful && (
                        <Fragment>
                            <Form
